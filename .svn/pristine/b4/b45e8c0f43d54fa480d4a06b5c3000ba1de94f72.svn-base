...
public class ChangeColorIconWithTextView extends View {
	...
	private Paint icobut;
	/**
	 * 颜色
	 */
	private int mColor = 0x00A2C9;
	/**
	 * 透明度 0.0-1.0
	 */
	...
	public void setStyl(String color,String text){
	...
	private void drawSourceText(Canvas canvas, int alpha) {
		...
		int i = 0;
		if(mIconBitmap != null){
			i = mIconRect.bottom + mTextBound.height();
		}else{
			i = 
//					mIconRect.bottom + mIconRect.height() / 2
//			- mTextBound.height() / 2;
					getHeight() / 2 + mTextBound.height() / 2;
		}
		canvas.drawText(mText, mIconRect.left + mIconRect.width() / 2
				- mTextBound.width() / 2,i
				, mTextPaint);
	}
	
	private void drawIocbut(Canvas canvas, int alpha) {
		icobut = new Paint();
		icobut.setColor(mColor);
		icobut.setStrokeWidth(10);
		icobut.setAlpha(alpha);
		float x = getHeight()-3;
		canvas.drawLine(0,x, getWidth(),x, icobut);
	}
