...
public class PayActivity {
	...
	public static WXPayResultCont payresu;

	public interface WXPayResultCont {
		/**
		 * 
		 * @param resultStatus
		 *            返回结果
	...
	public PayActivity(Context context, String APP_ID, String MCH_ID,
			String API_KEY) {
	 ...
	 *            订单价格
	 * @param order
	 *            自定义订单号，没有时传null，将随机生成
	 */
	public void pay(String body, String price, String order) {
		this.trade_no = order;
		this.body = body;
		try {
			int i = (int) (Double.valueOf(price) * 100);
			this.price = i + "";
	 ...
	 * @param noncestr 随机串
	 * @param timestamp 时间戳
	 * @param packagevla 财付通文档填写的数据和签名
	 * @param sign 微信开放平台文档对数据做的签名
	 */
	public void pay(String appid ,String prepayid,String partnerid,String noncestr,String timestamp,String packagevla,String sign){
		req.appId = appid;
		req.partnerId = partnerid;
		req.prepayId = prepayid;
		req.packageValue = packagevla;
	...
	public void setPayResultCont(WXPayResultCont paype) {
		payresu = paype;
	}
	...
	private String genProductArgs() {
		...
		try {
			...
			if (trade_no == null || trade_no.equals("")) {
				trade_no = genOutTradNo();
			}
			Log.i("trade_no", trade_no);
