...
//            toast.setText(msg);
     ...
     * @param gravity
     */
    public static void showTextToast(View showView, Context context, int gravity) {
        if (toast == null) {
    ...
    public static Dialog createChoiceDialog(Context context, String choCen, String cen1, String cen2, OnClickListener on1, OnClickListener on2) {
        return createChoiceDialog(context, choCen, cen1, cen2, 0, false, on1, on2);
    }
     ...
     * @param on1         第一个键点击事件(传null为关闭窗口)
     * @param on2         第二个键点击事件(传null为关闭窗口)
     * @param iconId      iconId 如果传0会隐藏icon
     * @param isOneChoice 是否是单选按钮，如果是true会隐藏第一个按钮
     * @return
     */
    public static Dialog createChoiceDialog(Context context, String choCen, String cen1, String cen2, int iconId, boolean isOneChoice, OnClickListener on1, OnClickListener on2) {
        closeDialog();
        dialog = new Dialog(context, R.style.mDialogB);
        LayoutInflater li = LayoutInflater.from(context);
        //如果有自定义页面，加载自定义
        View view = li.inflate(layoutId != 0 ? layoutId : R.layout.prompt_tool, null);
        TextView cen = view.findViewById(R.id.content);
        TextView qugg = view.findViewById(R.id.cancel);
        TextView queren = view.findViewById(R.id.confirm);
...
//			if(isMainActivityAlive(context))
    ...
    public static Dialog createChoiceOneDialog(Context context, String choCen, String cen1, OnClickListener on1) {
        return createChoiceDialog(context, choCen, cen1, null, 0, true, null, on1);
    }
